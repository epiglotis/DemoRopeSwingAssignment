...
public class IntVariable : GameVariable
{
    ...
    public void Devide(int divider) => savedValue = (useVariableLimits) ? Mathf.Clamp(Mathf.RoundToInt(savedValue / divider), minValue, maxValue) : Mathf.RoundToInt(savedValue / divider);

    ///<Summary> IntVariable ile bölme yapılmaktadır.
    /// Eğer bölümün sonucu ondalıklı ise en yakın olan tam sayıya tamamlanır.</Summary>
    public void Devide(IntVariable divider) => savedValue = (useVariableLimits) ? Mathf.Clamp(Mathf.RoundToInt(savedValue / divider.Value), minValue, maxValue) : Mathf.RoundToInt(savedValue / divider.Value);

    ///<Summary> IntReference ile bölme yapılmaktadır.
    /// Eğer bölümün sonucu ondalıklı ise en yakın olan tam sayıya tamamlanır.</Summary>
    public void Devide(IntReference divider) => savedValue = (useVariableLimits) ? Mathf.Clamp(Mathf.RoundToInt(savedValue / divider.Value), minValue, maxValue) : Mathf.RoundToInt(savedValue / divider.Value);
}